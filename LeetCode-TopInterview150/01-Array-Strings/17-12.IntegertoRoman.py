...
class Solution:
    def intToRoman(self, num: int) -> str:
        ...
        for v, s in zip(vals, syms):
            if num == 0:
                break
            count = num // v               # how many of this symbol to use
            if count:
                res.append(s * count)     # append symbol count times
                num -= v * count
        return "".join(res)
...
# Time Complexity: O(1) (constant, at most 13 iterations & ≤15 appends)
# Space Complexity: O(1) (constant auxiliary space, output size ≤15 chars)
