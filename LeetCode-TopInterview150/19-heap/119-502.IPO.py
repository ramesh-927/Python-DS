...
class Solution:
    def findMaximizedCapital(self, k, W, profits, capital):
        ...
        W: initial capital
        profits, capital: lists of length n
        """
        n = len(profits)
        # Pair up and sort by required capital
        projects = sorted(zip(capital, profits), key=lambda x: x[0])
        
        max_profit = []  # will store -profit to simulate max-heap
        i = 0  # pointer into sorted projects
        
        for _ in range(k):
            # push all projects affordable with current W into max-heap
