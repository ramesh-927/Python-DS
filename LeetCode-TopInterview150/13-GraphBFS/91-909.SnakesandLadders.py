...
class Solution:
    def snakesAndLadders(self, board):
        ...
        def index_to_pos(i):
            # Convert cell number (1-based) to row, col
            row = n - 1 - (i - 1) // n
            col = (i - 1) % n
            if (n - 1 - row) % 2 == 1:  # # Reverse direction on every alternate row
                col = n - 1 - col
            return row, col
        visited = set()
        queue = deque([(1, 0)])
        ...
        while queue:
            ...
            for step in range(1, 7):
                ...
                if next_square >  n * n:
                    continue
                r, c = index_to_pos(next_square)
                if board[r][c] != -1:    # snake/ladder exists
...
# Time: O(n^2) — each square is visited at most once; each visit checks up to 6 neighbors.

# Space: O(n^2) — visited array and BFS queue at worst store a constant fraction of board.