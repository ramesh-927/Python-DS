"""
Docstring for AppleInterview.04-Recurrsion.34-39.CombinationSum
...
if __name__ == "__main__":
    ...
    print(sol.combinationSum([2,3,5], 8))    # Prints : [[2,2,2,2],[2,3,3],[3,5]]
    print(sol.combinationSum([2], 1))        # Prints : []

# I used backtracking with DFS. Since elements can be reused, I pass the same index i when recursing, 
# and prune branches early when the sum exceeds the target to optimize performance.

# Time Complexity: O(N^(T/M))
# (where T = target, M = min(candidates), N = len(candidates))
