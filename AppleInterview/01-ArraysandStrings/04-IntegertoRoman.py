...
class Solution:
    def intToRoman(self, num):
        num = int(num)  # safely convert string → int
        
        # Values and symbols must match 1-to-1 and be in descending order
...
# I used a greedy approach with a predefined list of Roman values and symbols from largest to smallest.
#  We repeatedly pick the largest possible Roman symbol that fits into the remaining number — this 
# works because Roman numerals follow a strict subtractive pattern.

# Time Complexity: O(1) → because max 3999 needs at most ~13 characters (e.g., 3888 = MMMDCCCLXXXVIII)
