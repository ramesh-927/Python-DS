...
#  Time Complexity:  O(V + E) — visit each node and each edge once 
#  Space Compelxity:  O(V) extra for hashmap + recursion stack up to O(V) 

# I used DFS recursion with a hashmap to map original nodes to their clones so each node is copied 
# once and cycles are handled. This gives O(V+E) time and O(V) space.
